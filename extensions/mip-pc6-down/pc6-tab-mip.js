define('mip-plugindemo/mip-pc6-tab', ['require', 'customElement', 'zepto'], function(require) {
    // mip 组件开发支持 zepto
    var $ = require('zepto');
	...
	var tab=function(element){
			...
			if (hsId==0){
			    ...
			    var obj=$(element);
                    ...
                    oli.width(window_w);
					$("#tags-main").width(window_w);
                for(var i=1; i<oli.length; i++){ospan+='<span></span>'}
                $('.pagenum').html(ospan)
                oul.width(oli.length*oli.width());
				
                //点击li
                onav_li.eq(0).addClass('active');
                onav_li.click(function(){
					var i=$(this).index();
					iNow=i;
					$("body").scrollTop($('body').scrollTop()+1).scrollTop($('body').scrollTop()-1);
                    onav_li.eq(i).addClass('active').siblings().removeClass('active');
                    oul.css({'-webkit-transform':"translate3d(" + -window_w*i + "px, 0px, 0px)"});
                });
				...
				oul[0].addEventListener('touchmove', function(e){
					...
					if (touch.d==1){//左右滚动
					    e.preventDefault();
					    oul.css({'-webkit-transform':"translate3d(" + -(window_w*iNow-e.targetTouches[0].pageX+touch.s[0])+ "px, 0px, 0px)"});
					}
				}, false);
				...
				oul[0].addEventListener('touchend', function(e){
					if(touch.d==1){
						if((new Date()).getTime()-touch.s[2]>700){
							if (e.changedTouches[0].pageX-touch.s[0]>window_w/3){
								auto("right")
							} else if (touch.s[0]-e.changedTouches[0].pageX>window_w/3){
								auto("left")
							} else {
								auto("reset")
							}
				...
				function auto(dir){
				     ...
				     if(dir=="left"){
					     ...
					     oul.animate({'-webkit-transform':"translate3d(" + -window_w*iNow + "px, 0px, 0px)"});
					 }else if (dir=="reset"){
						oul.animate({'-webkit-transform':"translate3d(" + -window_w*iNow + "px, 0px, 0px)"});
					 }else if (dir=="right"){
						iNow<=0?iNow=0:iNow--; 
				...
				//解决横向滚动图片无法出现
				$("#wrapper2")[0].addEventListener('touchend', function(e){bugImg()},false);
				$("#wrapper4")[0].addEventListener('touchend', function(e){bugImg()},false);
...
require(['mip-plugindemo/mip-pc6-tab'], function (plugindemo) {
    MIP.registerMipElement('mip-pc6-tab', plugindemo);
});